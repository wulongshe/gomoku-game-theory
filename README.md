...
- **同时落子**：每回合 30/60 秒，双方都提交或时间到后同时揭晓。对方提交前仍可变更落点；超时未提交则本回合弃着。
- **撞点**：双方落在同一点时，按开局选择的模式处理——
  - 禁点模式：该点变为禁点，双方都无法再落子；
  - 半子模式：化作太极半子，双方各占一半（计分各算 0.5）。
- **胜负**：任意方向连续累计满 5 即胜（半子算 0.5，5 或 5.5 都算赢）。双方同时连五则连线棋子一起消失，对局继续。
- **对抗性规则**：首回合只能落在中央 3×3 且禁天元；禁点连成五整线清除，位置重新可用。

支持建房间发链接开黑，或按时限 + 模式随机匹配。
