...
- **撞子**：双方落在同一点时，该点变为禁点，双方都无法再落子。
- **胜负**：任意方向连续五子即胜。双方同时连五则连线棋子一起消失，对局继续。
- **对抗性规则**：首回合只能落在中央 3×3 且禁天元；禁点连成五整线清除，位置重新可用。

## 功能

- **双人联机**：建房间发链接开黑，或随机匹配。随机匹配每天 20:00–22:00（北京时间）开放，每回合时限从 10s 起逐回合放宽到 45s；局终可发起「再来一局」。
- **人机对战**：四档难度（简单 / 普通 / 困难 / 大师），AI 在 Web Worker 中搜索，不阻塞界面。
- **中秋特辑 · 月饼大乱斗**：3~5 人同场、19 路棋盘、每回合 60s，棋子为五色月饼。连成五即完赛并按先后排名，多人同回合连五则一起消失；只剩一人或无人能连五时终局。
...
| `MATCH_WINDOW` | 随机匹配开放时段（北京时间），默认 `20:00-22:00`，起止相同为全天开放 |
