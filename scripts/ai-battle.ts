// 由 Node 原生运行 TypeScript（Node ≥ 23.6 默认开启 type stripping，无需 tsx）：pnpm ai:battle
import { Worker } from 'node:worker_threads'
import { createGame, settleFrame, type GameMode, type GameState, type Point, type Seat } from '../src/engine/game.ts'
import { type SideConfig } from './ai-battle-search.ts'
...
// 2 × 并行局数 个 Worker 轮询承接每次独立搜索：单局墙钟约两方预算的较大值，多局并行叠加。
function createWorkerPool(workerFile: URL, size: number): WorkerPool {
  const workers = Array.from({ length: size }, () => new Worker(workerFile))
  const pending = new Map<number, (move: Point | null) => void>()
  let nextId = 0
  let turn = 0
  for (const worker of workers) {
    worker.on('message', ({ id, move }: SearchResponse) => {
  ...
  return {
    search(state, seat, side) {
      return new Promise((resolve) => {
        ...
        workers[turn++ % workers.length].postMessage({ id, state, seat, side })
      })
    },
    close() {
...
async function runSeries(config: MatchConfig, search: SearchFn): Promise<MatchSummary> {
  ...
  async function playRound(i: number): Promise<void> {
    ...
    console.log(`${prefix}开局：黑 ${formatSide(config.black)} vs 白 ${formatSide(config.white)}（模式 ${config.mode}）`)
    ...
    console.log(`${prefix}本局 ${formatOutcome(outcome)}（用时 ${seconds}s）｜${formatSummaryLine(summary, config)}`)
  }
...
function printSummary(config: MatchConfig, summary: MatchSummary, elapsedMs: number): void {
  ...
  console.log(`黑方：${formatSide(config.black)}，胜 ${summary.blackWins}，负 ${summary.whiteWins}，平 ${summary.draws}`)
  console.log(`白方：${formatSide(config.white)}，胜 ${summary.whiteWins}，负 ${summary.blackWins}，平 ${summary.draws}`)
  console.log(
  ...
  console.log(`总用时：${(elapsedMs / 1000).toFixed(1)}s`)
}
...
async function main(): Promise<void> {
  ...
  const pool = createWorkerPool(new URL('./ai-battle-worker.ts', import.meta.url), config.parallel * 2)
  const startedAt = performance.now()
  try {
